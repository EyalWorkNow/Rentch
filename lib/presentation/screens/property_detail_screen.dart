...
class _PropertyDetailScreenState extends State<PropertyDetailScreen> {
  ...
  // Boost ("הקפצת מודעה") from the landlord's property page. Subscription-gated
  // with clearing: no active subscription → the paywall (סליקה); otherwise a
  // confirm sheet, the server boost call, and a result snackbar.
  Future<void> _handleBoost(RentalProperty p) async {
    final provider = context.read<DatingProvider>();
    ...
    final choice = await showModalBottomSheet<_BoostChoice>(
      ...
      builder: (ctx) => _BoostOptionsSheet(
        hasQuota: sub.canBoost,
        remaining: sub.boostRemaining ?? 0,
        unlimited: sub.boostUnlimited,
      ),
    );
    ...
    switch (choice) {
      case _BoostChoice.regularQuota:
        await _runQuotaBoost(p, provider, snack);
      case _BoostChoice.regularPaid:
        await _runPaidBoost(p, provider, snack,
  ...
  Future<void> _runQuotaBoost(RentalProperty p, DatingProvider provider,
      void Function(String, {bool ok}) snack) async {
  ...
  // checkout → verify → stamp the tier locally.
...
/// monthly quota is spent it becomes the "out of boosts, buy one" popup.
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              hasQuota ? 'הקפצת המודעה' : 'נגמרו ההקפצות שלך החודש',
...
class _BoostOptionCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Material(
      color: gold ? const Color(0xFFFFFBEB) : AppColors.slate50,
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(18),
            border: Border.all(
                color: gold ? AppColors.amber : AppColors.slate200,
                width: gold ? 1.5 : 1),
          ),
          child: Row(
            children: [
              ...
              Container(
                ...
                child: Text(trailing,
                    style: TextStyle(
                        fontSize: 14,
                        fontWeight: FontWeight.w900,
                        color: trailingIsPrice ? Colors.white : accent)),
              ),
            ],
          ),
        ),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          ...
                          isAgency ? 'מתווך נדל"ן' : 'בעל נכס פרטי',
                          style: TextStyle(
                            fontSize: 12,
                            fontWeight: FontWeight.w700,
                            color: isAgency
