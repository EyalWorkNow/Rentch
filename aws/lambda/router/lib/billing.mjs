...
export const BILLING = {
  ...
  plans: {
    monthly: { id: 'monthly', priceAgorot: 4500,  months: 1,  label: 'חודשי' },
    annual:  { id: 'annual',  priceAgorot: 45000, months: 12, label: 'שנתי'  },
  },
...
// ── boost ("הקפצת מודעה") ─────────────────────────────────────────────────────
// A boosted listing surfaces at the top of the tenant feed for BOOST_DAYS. The
// monthly quota per plan covers REGULAR boosts (×2 exposure). Ultra (×5) is
// always a paid one-off. Only entitled users get the free quota.
export const BOOST_DAYS = 7;
export function boostQuota(planId) {
  return ({ monthly: 2, annual: 5 })[planId] ?? 0;
}
// Current boost usage for `sub` at `now` (ms), month-scoped (resets each month).
export function boostState(sub, now) {
  const entitled = hasActiveEntitlement(sub, now);
  const quota = entitled ? boostQuota(sub?.plan) : 0;
  const month = new Date(now).toISOString().slice(0, 7); // YYYY-MM
  const used = sub?.boostMonth === month ? (sub.boostsUsed || 0) : 0;
  const remaining = quota === Infinity ? Infinity : Math.max(0, quota - used);
  return { quota, used, remaining, month, days: BOOST_DAYS };
}

// Promo coupons — SERVER-AUTHORITATIVE (the client never sends a price; it sends
// a code, we decide the discounted price). Keyed by lowercased code.
//   ea35 → monthly at ₪1 (launch/test promo). `expiresAt` caps the blast radius
//   of a leaked code: past that instant it stops working for NEW signups
//   (existing subscribers keep the promo). Bump the date to extend the promo.
export const COUPONS = {
  ea35: { plan: 'monthly', priceAgorot: 100, expiresAt: '2026-08-31T23:59:59Z' },
};

// Server-authoritative one-off product prices (agorot). The client NEVER sends
// the charge amount — it names a product and the SERVER prices it. Without this,
// a crafted request could pay ₪0.01 for a paid unlock.
export const ONE_OFF_PRODUCTS = {
  contract: { priceAgorot: 5000, description: 'שיפור חוזה עם AI — Rently' }, // ₪50
  // Paid boosts (used when the subscription quota is spent, or for Ultra which
  // is never included). `tier`/`boostDays` drive the server apply-on-payment.
  boost_regular: { priceAgorot: 1000, tier: 'regular', boostDays: BOOST_DAYS, description: 'הקפצת מודעה — Rently' }, // ₪10
  boost_ultra:   { priceAgorot: 5000, tier: 'ultra',   boostDays: BOOST_DAYS, description: 'הקפצת Ultra — Rently' },  // ₪50
};

export function resolveOneOffProduct(product) {
  if (!product) return null;
  return ONE_OFF_PRODUCTS[String(product).trim().toLowerCase()] || null;
}

// Boost metadata for a one-off product ({ tier, boostDays, priceAgorot }) or
// null if the product isn't a paid boost. Used by the apply-on-payment path.
export function boostProductMeta(product) {
  const p = resolveOneOffProduct(product);
  return p && p.tier ? { tier: p.tier, boostDays: p.boostDays || BOOST_DAYS, priceAgorot: p.priceAgorot } : null;
}

// Resolve a coupon code against the chosen plan at time `now` (ms). Returns
// { code, priceAgorot } when valid, applicable to that plan, and not expired;
// else null (unknown code, wrong plan, or past its expiry).
export function resolveCoupon(code, planId, now = Date.now()) {
  if (!code) return null;
  const norm = String(code).trim().toLowerCase();
  const c = COUPONS[norm];
  if (!c) return null;
  if (c.plan && c.plan !== planId) return null; // coupon is plan-scoped
  if (c.expiresAt && Date.parse(c.expiresAt) <= now) return null; // expired
  return { code: norm, priceAgorot: c.priceAgorot };
}
...
export function entitlementView(sub, activeCount, now) {
  ...
  return {
    ...
    // Boost quota for the "הקפצת מודעה" button (null = unlimited / VIP).
    boost: (() => {
      const bs = boostState(sub, now);
      return {
        quota: bs.quota === Infinity ? null : bs.quota,
        used: bs.used,
        remaining: bs.remaining === Infinity ? null : bs.remaining,
        unlimited: bs.quota === Infinity,
        days: bs.days,
      };
    })(),
  };
