...
//
// IMPORTANT: /clients/search filters by the SINGLE `email` field. The plural
// `emails: [...]` is NOT a search filter — Morning ignores it and returns
// arbitrary clients, which would (a) never find the real client → duplicate
// creates, and (b) risk returning SOMEONE ELSE'S client id. Verified live.
async function findClientIdByEmail(email) {
  if (!email) return null;
  const found = await api('/clients/search', { body: { email, page: 1, pageSize: 5 } });
  const items = found.items || found.data?.items || [];
  // Defense-in-depth: only accept a row whose emails actually contain ours.
  const hit = items.find((c) => (c.emails || []).includes(email)) || items[0];
  return hit?.id || null;
}
...
export async function upsertClient({ name, email, taxId }) {
  if (email) {
    try {
      const id = await findClientIdByEmail(email);
      if (id) return id;
  ...
  try {
    ...
    // errorCode 1010 = "a request with identical data already exists". Morning
    // dedups by NAME (not email), so a landlord whose name already exists as a
    // client — even one with NO email — triggers this. Morning helpfully returns
    // the EXISTING client's id in `errorMessage`, so use it directly. (A
    // name-only client has no email, so an email re-search would miss it — that's
    // why the earlier fix still 502'd.) Fall back to email search otherwise.
    if (e?.data?.errorCode === 1010) {
      const existingId = String(e.data.errorMessage || '').trim();
      if (/^[0-9a-f-]{32,40}$/i.test(existingId)) return existingId; // it's a UUID
      if (email) {
        const id = await findClientIdByEmail(email);
        if (id) return id;
      }
...
export async function createPaymentForm({
  plan, clientId, clientName, clientEmail, group, successUrl, failureUrl, notifyUrl, subscriptionId,
}) {
  const c = cfg();
  ...
  const body = {
    ...
    group: resolveGroup(group),      // chosen method (card/Bit/Apple/Google) or terminal default
    client: {
...
export async function createPaymentFormOneOff({
  amountIls, description, clientId, clientName, clientEmail, custom, group,
...
// ── verify-on-return (IPN-independent activation) ────────────────────────────
// The hosted-page IPN format is unverified, so we ALSO confirm a payment by
// asking Morning directly: search the business's tax-receipts (type 320) issued
// to this client since `sinceIso`. A match means the hosted payment completed.
// Returns the newest match (or null).
export async function findRecentPaidDocument({ clientId, sinceIso, amountAgorot } = {}) {
  if (!clientId) return null;
  const out = await api('/documents/search', {
    body: {
      clientId,
      type: [DOC_TYPE.INVOICE_RECEIPT], // 320 = חשבונית מס/קבלה
      fromDate: sinceIso,               // YYYY-MM-DD
      sort: 'documentDate',
      pageSize: 10,
    },
  });
  const items = out.items || out.data?.items || [];
  if (!items.length) return null;
  // newest first (creationDate is a unix ts)
  items.sort((a, b) => Number(b.creationDate || 0) - Number(a.creationDate || 0));
  const toAgorot = (d) => {
    const amount = Number(d.amount ?? d.amountLocal ?? 0);
    return Number.isFinite(amount) ? Math.round(amount * 100) : null;
  };
  // When an expected amount is given (e.g. a ₪10/₪50 boost vs a ₪45 subscription),
  // pick the newest receipt that MATCHES it — otherwise a boost confirm could
  // latch onto an unrelated subscription payment.
  const d = amountAgorot != null
    ? items.find((it) => toAgorot(it) === amountAgorot)
    : items[0];
  if (!d) return null;
  return {
    docId: d.id || null,
    number: d.number || null,
    amountAgorot: toAgorot(d),
    docUrl: d.url?.he || d.url?.origin || null,
    transactionId: String(d.id || ''), // the document id is a stable idempotency key
  };
}

// Find the saved credit-card token for this client — used to charge monthly
// renewals. Returns { token, cardLast4, cardBrand } or null.
export async function findCardToken({ clientId }) {
  if (!clientId) return null;
  const out = await api('/payments/tokens/search', {
    body: { externalKey: clientId, page: 1, pageSize: 5 },
  });
  const items = out.items || out.data?.items || [];
  if (!items.length) return null;
  const t = items[0];
  return {
    token: t.id || null,
    cardLast4: t.number || t.paymentNumber || null,
    cardBrand: t.cardType != null ? String(t.cardType) : null,
  };
}
...
export function parseWebhook(payload = {}) {
  ...
  // Subscription: "<uid>::<planId>". One-off boost: "<uid>::oneoff::<product>::<propertyId>".
  const parts = rawCustom.split('::');
  const subId = parts[0];
  const isOneOff = parts[1] === 'oneoff';
  const planId = isOneOff ? 'oneoff' : parts[1];
  const oneOffProduct = isOneOff ? (parts[2] || null) : null;
  const oneOffPropertyId = isOneOff ? (parts[3] || null) : null;
